...
class Board: CustomStringConvertible {
    ...
    init(dimensions: Int, mines: Int) throws {
        ...
        guard mines <= dimensions * dimensions else {
            throw Errors.tooManyMines
        }
        self.cheat = false
        self.mines = mines
        self.dimensions = dimensions
        self.pieces = Array(repeating: Piece.covered, count: dimensions * dimensions)
    ...
    private func offset(_ tup: (Int, Int)) -> Int {
        offset(tup.0, tup.1)
    }
    
    fileprivate func piece(x: Int, y: Int) -> Piece? {
    ...
    var description: String {
        var str = ""
        (0..<dimensions).forEach { y in
            (0..<dimensions).forEach { x in
    ...
    fileprivate func floodFill(x: Int, y: Int) -> [(Int, Int)] {
        ...
        var visited = [Bool](repeating: false, count: dimensions * dimensions)
        var queue: [(Int, Int)] = []
        var result: [(Int, Int)] = []
        queue += [(x, y)]
        while !queue.isEmpty {
...
struct GameCommand {
    ...
    private func new(params: [Int]) throws -> Game {
        ...
        let (dimensions, mines) = (params[0], params[1])
